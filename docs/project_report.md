...
> **PPT 与演示视频**：百度网盘链接 [https://pan.baidu.com/s/1m0TJGKHL-pUq0UYzWzF-sw?pwd=4513](https://pan.baidu.com/s/1m0TJGKHL-pUq0UYzWzF-sw?pwd=4513) 提取码：`4513`
...
![session isolation](assets/session-isolation.png)
...
第五阶段是验收整理。我们把功能、并发、性能、稳定性相关的测试程序统一放到 `tools/` 目录，并完善 CMake，使核心 target 可以通过标准 CMake 构建；同时补充 README 和项目报告，让评审能够从仓库入口直接看到题目编号、项目目标、构建方式、测试方式和最终结果。
...
项目仓库按功能模块组织，核心代码、通信协议、共享数据结构、测试工具和项目文档分别放在不同目录中，便于构建、阅读和验收。主要目录如下：
...
`tools/` 目录中的程序对应功能 smoke、性能 benchmark、并发验证、跨 session 隔离、ring 压力和负例稳定性等验收场景，和 `tools/scripts/run_acceptance_validation.py`、`tools/scripts/run_cross_session_sync_test.py` 一起组成项目的测试入口。
