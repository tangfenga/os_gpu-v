...
系统整体由四部分组成：client 代理库、gRPC 协议层、server 资源管理与执行层、shared memory/ring fast path。整体架构如下图所示：

![os_gpu-v system architecture](assets/system-architecture.svg)
...
![os_gpu-v session model](assets/session-model.svg)
...
![os_gpu-v data paths](assets/data-paths.svg)
...
第六个关键进展是补齐负例和稳定性测试。GPU 虚拟化原型不能只在正常输入下工作，还要能处理 invalid pointer、double free、invalid stream/event 等错误。我们加入 `stability_negative_smoke.cu`，确认代理库遇到伪造 handle 或重复释放时返回 CUDA 错误码，而不是在 server 中误解引用非法地址。稳定性测试连续运行 10 分钟，覆盖多轮 API 调用、资源创建释放和数据传输，最终失败数为 0。

## 6. 系统测试情况

测试程序集中放在 `tools/` 目录，保留的都是和验收直接相关的工具。`runtime_query_smoke.cu` 用于检查 Runtime/Driver 基础查询；`vector_add_smoke.cu` 验证 malloc、H2D、kernel launch、D2H、free 的完整闭环；`memcpy_baseline.cu` 覆盖 H2D、D2H 和 D2D 三种拷贝；`matrix_mul_benchmark.cu` 既用于功能验证，也用于端到端性能对比。

stream 和 event 相关测试由 `stream_async_smoke.cu`、`event_timing_smoke.cu` 和 `host_alloc_smoke.cu` 覆盖。它们主要验证异步拷贝、stream 同步、event record/query/synchronize、event timing 以及 host allocation 路径是否符合 CUDA Runtime 的基本语义。

并发和隔离测试由 `concurrent_worker.cu`、`cross_session_sync_test.cu` 和 `run_cross_session_sync_test.py` 覆盖。双进程并发测试会同时启动两个通过 proxy 运行的 CUDA 进程，组合包括 vector+vector、vector+matmul、matmul+matmul，用来观察两个 session 是否可以并发运行、结果是否正确、资源是否串扰。跨 session 同步测试中，holder 进程提交长 kernel，probe 进程随后执行 `cudaDeviceSynchronize()`。结果显示 probe 不会等待 holder 的长 kernel，说明 session 同步隔离生效。

压力和负例测试由 `fire_forget_stress.cu`、`stability_negative_smoke.cu` 和 `run_acceptance_validation.py` 覆盖。`fire_forget_stress.cu` 主要压 ring wrap-around 和后台 worker 消费；`stability_negative_smoke.cu` 检查 double free、invalid device pointer、invalid stream、invalid event 等错误路径；acceptance 脚本把功能测试、并发测试、性能测试和稳定性测试串起来，减少人工重复操作。
